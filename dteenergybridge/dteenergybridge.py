...
class DteEnergyBridge:
    ...
    def __init__(self, host, bridge_version=1, timeout=5):
        ...
        if bridge_version not in (1, 2):
            raise exceptions.InvalidArgumentError(
                'Bridge version must be either 1 or 2')
    ...
    def instantaneous_demand_path(self):
        ...
        return ('instantaneousdemand'
                if self.bridge_version == 1
                else 'zigbee/se/instantaneousdemand')

    def get_current_energy_usage(self):
        """Returns the current energy usage in kW"""
        try:
            response = requests.get(
                self.base_url + self.instantaneous_demand_path(),
        ...
        if response.status_code != 200:
            raise exceptions.InvalidResponseError(
                "Received status {} from DTE Energy Bridge".format(
        ...
        if len(response_split) != 2:
            raise exceptions.InvalidResponseError(
                "Received bad response '{}' from DTE Energy Bridge".format(
                    response.text))
